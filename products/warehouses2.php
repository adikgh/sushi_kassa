<? include "../config/core.php";
   ...
   if (mysqli_num_rows($sql)) {
      ...
      $ln = substr($ln, 0 , -2); $filter = true;
   }

	// filter
	if ($filter) $product_all = db::query("select * from product_item where id in ($ln)");
	else $product_all = db::query("select * from product_item");
	...
	// filter
	if ($filter) $product = db::query("select * from product_item where id in ($ln) order by ins_dt desc limit $page_start, $page_age");
	else $product = db::query("select * from product_item order by ins_dt desc limit $page_start, $page_age");
                        ...
                        <a class="uc_uiln" href="/products/item/view/?id=<?=$item_d['id']?>">
                           ...
                           <div class="uc_uinu">
                              ...
                              <div class="uc_ui_cont">
								...
								<div class="uc_uin_other"><?=(product::pr_color($item_d['color_id']))['name_ru']?>, <?=(product::pr_size($item_d['size_id']))['name']?></div>
